...
if __name__ == "__main__":
    ...
    parser.add_argument("CWD", help="Directory that HAC was kicked off from", type=str)
    parser.add_argument("Tool_Dir", help="Directory that HAC resides in", type=str)
    args    = parser.parse_args()
    
    cmd_history     = []
    IP              = args.IP
    SName           = "HAC_{}".format(args.LastOct)
    CWD             = "{}/".format(args.CWD)
    ...
    while True:
        try:
            ...
            if click.confirm("\nDo you want to exit HAC?", default=False):
                print("Later Tater")
                exit()
            else:
                continue
